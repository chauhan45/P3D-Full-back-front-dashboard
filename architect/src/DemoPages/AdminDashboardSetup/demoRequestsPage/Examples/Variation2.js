...
export default class AdminDashboardDemoRequestsPage extends Component {  
  constructor(props){
    super(props)
    this.state = {
      users: [],
      loading:true,    
      demoRequestSearchInput:"",
      demoRequestSearchFieldEmptyError: false,
      demoRequestSearchNotFoundError: false,   
    }        
    ...
    this.demoRequestSearch = this.demoRequestSearch.bind(this);
    this.demoRequestSearchInputChange = this.demoRequestSearchInputChange.bind(this);
  }
  async getUsersData(){    
    const res = await axios.post("/admin/fetch/customer/requestDemo")                           
  ...
  demoRequestSearchInputChange(e){
    this.setState({demoRequestSearchInput:e.target.value})
  }
  demoRequestSearch(){   
    this.setState({demoRequestSearchFieldEmptyError:false});
    this.setState({demoRequestSearchNotFoundError:false});
    fetch("/backend/admin/fetch/demoRequest",{
      method:"POST",      
      body: JSON.stringify({
        email:this.state.demoRequestSearchInput
      }),
      headers: {
        ...
        if(data === "noEmail"){
          this.setState({demoRequestSearchFieldEmptyError:true});             
          return;
        }
        else if(data === "notFound"){
          this.setState({demoRequestSearchNotFoundError:true});                 
          return;
        }
        else{
          for(let key in data){
    ...
    this.setState({demoRequestSearchInput:""});
  }
  componentDidMount(){
    this.getUsersData();      
  }  
  render() {
    ...
    return (
      <>          
        <h1 id="adminHeading" className="text-center">Demo Requests</h1>
        
        {/* Search */}
        <div className='container row mt-2'>
          <div className='col-10'>
            <input value={this.state.demoRequestSearchInput} onChange={this.demoRequestSearchInputChange} className='form-control w-100' type="search" placeholder='Search by email'/>
          </div>
          <div className='col-2'>
            <input className='form-control' type="submit" value='Search' onClick={this.demoRequestSearch}/>
          </div>
        </div>      
        <div className='container row mt-2'>
            {this.state.demoRequestSearchFieldEmptyError&&<h5 className="col-12" style={{color:"red"}}>Field is empty</h5>}
            {this.state.demoRequestSearchNotFoundError&&<h5 className="col-12" style={{color:"red"}}>Email not found</h5>}
        </div>
        {/* Search */}
