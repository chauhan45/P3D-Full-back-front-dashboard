...
export default class AdminDashboardAllAdminsPage extends Component {  
  constructor(props){
    super(props)
    this.state = {
      users: [],
  ...
  async getUsersData(){    
    const res = await axios.post("/admin/fetch/admins")                           
  ...
  sureButton(){    
    ...
    if(this.sureButtonRef0.current.title.split("_")[0]==="sureApprovalButton"){
      ...
      $("#firstNameError").css("display","none");$("#lastNameError").css("display","none");$("#mobileNumberError").css("display","none");$("#emailError").css("display","none");$("#passwordError").css("display","none");$("#confirmPasswordError").css("display","none");$("passwordMatchError").css("display","none");
      if($("#firstNameInput").val()===""){
        ...
        axios.post("/backend/admin-edit-customer",
          {id:id1,first_name:$("#firstNameInput").val(),last_name:$("#lastNameInput").val(),mobile_number:$("#mobileNumberInput").val(),email:$("#emailInput").val(), password:$("#passwordInput").val()});                 
          $("#firstNameInput").val("");
          $("#lastNameInput").val("");
  ...
  render() {
    const columns = [             
      ...
      //   Cell: (data) => (<div style={{margin:"auto"}}>
      ...
      //     >Archived</div>), 
      //   accessor: 'is_deleted',  
      //   Cell: (data) => (<div style={{margin:"auto"}}><input type="checkbox" id={"is_deleted_"+data.original._id} defaultChecked={data.value} disabled/></div>),
      //   width: 70        
      // },
      {  
        Header: () => (
    ...
    return (
      <>                
        <h1 id="adminHeading" className="text-center">All Admins</h1>
