...
export default class AdminDashboardAllUnApprovedCustomers extends Component {  
  constructor(props){
    super(props)
    this.state = {
      users: [],
  ...
  async getUsersData(){    
    const res = await axios.post("/admin/fetch/unapproved/customers")                           
  ...
  sureButton(){    
    ...
    if(this.sureButtonRef0.current.title.split("_")[0]==="sureApproveButton"){
      ...
      axios.post("/backend/admin/edit/approve/customer",data0)
           .then(result=>console.log(result))
           .catch(error=>console.log(error));      
      document.getElementById("controlDivision_"+id0).parentElement.parentElement.remove();         
      $(".confirmationPopup").css("display","none");
  ...
  render() {
    ...
    return (
      <>                
        <h1 id="adminHeading" className="text-center">All Unapproved Customers</h1>
