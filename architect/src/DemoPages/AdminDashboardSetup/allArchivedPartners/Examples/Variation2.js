...
export default class AdminDashboardAllArchivedPartners extends Component {  
  constructor(props){
    super(props)
    this.state = {
      users: [],
      loading:true,     
      allArchivedPartnersSearchInput:"",
      allArchivedPartnersSearchFieldEmptyError: false,
      allArchivedPartnersSearchNotFoundError: false,                
    }        
    ...
    this.allArchivedPartnersSearch = this.allArchivedPartnersSearch.bind(this);
    this.allArchivedPartnersSearchInputChange = this.allArchivedPartnersSearchInputChange.bind(this);
  }
  async getUsersData(){    
    const res = await axios.post("/admin/fetch/partners/archived")                           
  ...
  sureButton(){    
    ...
    if(this.sureButtonRef0.current.id.split("_")[0]==="sureApprovalButton"){
      ...
      fetch('/backend/admin/edit/partner/unarchive',
        {   method: "POST", 
            'credentials': 'include',
  ...
  allArchivedPartnersSearchInputChange(e){
    this.setState({allArchivedPartnersSearchInput:e.target.value})
  }
  allArchivedPartnersSearch(){   
    this.setState({allArchivedPartnersSearchFieldEmptyError:false});
    this.setState({allArchivedPartnersSearchNotFoundError:false});
    fetch("/backend/admin/fetch/allArchivedPartners/search",{
      method:"POST",      
      body: JSON.stringify({
        email:this.state.allArchivedPartnersSearchInput
      }),
      headers: {
          'Content-type': 'application/json; charset=UTF-8'
      }
        ...
        if(data === "noEmail"){
          this.setState({allArchivedPartnersSearchFieldEmptyError:true});             
          return;
        }
        else if(data === "notFound"){
          this.setState({allArchivedPartnersSearchNotFoundError:true});                 
          return;
        }
        else{          
          for(let key in data){
    ...
    this.setState({allArchivedPartnersSearchInput:""});
  }
  ...
  render() {
    ...
    return (
      <>          
        <h1 className="text-center">All Archived Partners</h1>

        {/* Search */}
        <div className='container row mt-2'>
          <div className='col-10'>
            <input value={this.state.allArchivedPartnersSearchInput} onChange={this.allArchivedPartnersSearchInputChange} className='form-control w-100' type="search" placeholder='Search by email'/>
          </div>
          <div className='col-2'>
            <input className='form-control' type="submit" value='Search' onClick={this.allArchivedPartnersSearch}/>
          </div>
        </div>      
        <div className='container row mt-2'>
            {this.state.allArchivedPartnersSearchFieldEmptyError&&<h5 className="col-12" style={{color:"red"}}>Field is empty</h5>}
            {this.state.allArchivedPartnersSearchNotFoundError&&<h5 className="col-12" style={{color:"red"}}>Email not found</h5>}
        </div>
        {/* Search */}
