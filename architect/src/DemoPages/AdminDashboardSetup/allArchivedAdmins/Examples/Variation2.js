...
export default class AdminDashboardAllArchivedAdmins extends Component {  
  constructor(props){
    super(props)
    this.state = {
      users: [],
      loading:true,  
      allArchivedAdminsSearchInput:"",
      allArchivedAdminsSearchFieldEmptyError: false,
      allArchivedAdminsSearchNotFoundError: false,           
    }        
    ...
    this.allArchivedAdminsSearch = this.allArchivedAdminsSearch.bind(this);
    this.allArchivedAdminsSearchInputChange = this.allArchivedAdminsSearchInputChange.bind(this);

  }
  async getUsersData(){    
    const res = await axios.post("/admin/fetch/admins/archived")                           
  ...
  adminApproved(id0){                     
    ...
    let firstName = $("#admin_approved_"+id0).attr("class").split("_")[1];
    let lastName = $("#admin_approved_"+id0).attr("class").split("_")[2];
  ...
  allArchivedAdminsSearchInputChange(e){
    this.setState({allArchivedAdminsSearchInput:e.target.value})
  }
  allArchivedAdminsSearch(){   
    this.setState({allArchivedAdminsSearchFieldEmptyError:false});
    this.setState({allArchivedAdminsSearchNotFoundError:false});
    fetch("/backend/admin/fetch/allArchivedAdmins/search",{
      method:"POST",      
      body: JSON.stringify({
        email:this.state.allArchivedAdminsSearchInput
      }),
      headers: {
          'Content-type': 'application/json; charset=UTF-8'
      }
        ...
        if(data === "noEmail"){
          this.setState({allArchivedAdminsSearchFieldEmptyError:true});             
          return;
        }
        else if(data === "notFound"){
          this.setState({allArchivedAdminsSearchNotFoundError:true});                 
          return;
        }
        else{          
          for(let key in data){
    ...
    this.setState({allArchivedAdminsSearchInput:""});
  }
  okayButton(){
  ...
  render() {
    ...
    return (
      <>            
        ...
        <h1 className="text-center">All Archived Admins</h1>

        {/* Search */}
        <div className='container row mt-2'>
          <div className='col-10'>
            <input value={this.state.allArchivedAdminsSearchInput} onChange={this.allArchivedAdminsSearchInputChange} className='form-control w-100' type="search" placeholder='Search by email'/>
          </div>
          <div className='col-2'>
            <input className='form-control' type="submit" value='Search' onClick={this.allArchivedAdminsSearch}/>
          </div>
        </div>      
        <div className='container row mt-2'>
            {this.state.allArchivedAdminsSearchFieldEmptyError&&<h5 className="col-12" style={{color:"red"}}>Field is empty</h5>}
            {this.state.allArchivedAdminsSearchNotFoundError&&<h5 className="col-12" style={{color:"red"}}>Email not found</h5>}
        </div>
        {/* Search */}
