...
import EditPopup2 from '../../component/editPopup2';
...
export class users extends Component {
  constructor(props){
    ...
    this.state = {
      users: [],
      loading:true,
      allCustomersSearchInput:"",
      allCustomersSearchFieldEmptyError: false,
      allCustomersSearchNotFoundError: false,                
    }  
    ...
    // this.editCustomer1 = this.editCustomer1.bind(this);
    ...
    this.allCustomersSearch = this.allCustomersSearch.bind(this);
    this.allCustomersSearchInputChange = this.allCustomersSearchInputChange.bind(this);
  }
  async getUsersData(){    
    const res = await axios.post("/partnerDashboard/fetch/customers",{ withCredentials: true })                           
  ...
  sureButton(){        
    ...
    if(this.sureButtonRef0.current.title.split("_")[0]==="sureApprovalButton"){
      ...
      $("#firstNameError").css("display","none");$("#lastNameError").css("display","none");$("#mobileNumberError").css("display","none");$("#emailError").css("display","none");$("#companyNameError").css("display","none");$("#streetAddressError").css("display","none");$("#cityError").css("display","none");$("#stateError").css("display","none");$("#zipCodeError").css("display","none");$("#companyTypeError").css("display","none");$("#industryTypeError").css("display","none");
      ...
      $("#editCustomerPopup").css("display","none");
    }
    else{
      return;
    }             
  ...
  editCustomer0(id){
    window.scrollTo(0,0);
    $("#editCustomerPopup").attr("style","display:block !important;left:475px;z-index:1;height:807px");     
  ...
  allCustomersSearchInputChange(e){
    this.setState({allCustomersSearchInput:e.target.value})
  }
  allCustomersSearch(){   
    this.setState({allCustomersSearchFieldEmptyError:false});
    this.setState({allCustomersSearchNotFoundError:false});
    fetch("/backend/partnerDashboard/search/allCustomers/",{
      method:"POST",      
      body: JSON.stringify({
        email:this.state.allCustomersSearchInput
      }),
      headers: {
        ...
        if(data === "noEmail"){
          this.setState({allCustomersSearchFieldEmptyError:true});             
          return;
        }
        else if(data === "notFound"){
          this.setState({allCustomersSearchNotFoundError:true});                 
          return;
        }
        else{          
          for(let key in data){
    ...
    this.setState({allCustomersSearchInput:""});
  }
  componentDidMount(){
    this.getUsersData();    
  }  
  render() {
    const columns = [             
      ...
      {  
        ...
        Cell: (data) => <div className='checkBoxDivisions'><input type="checkbox" checked={data.value} disabled/></div>,
        width: 130
      },        
      // {  
      //   Header: () => (
      ...
      //   Cell: (data) => (<div className='checkBoxDivisions'>
      //                       <input className={"admin_approve_popup_"+data.original.firstName+"_"+data.original.lastName}
      //                              type="checkbox" onClick={()=>this.adminApproved(data.original._id)} 
      ...
      //     >Archived</div>), 
      //   accessor: 'is_deleted',  
      //   Cell: (data) => (<div className='checkBoxDivisions'> <input type="checkbox" id={"is_deleted_"+data.original._id} defaultChecked={data.value} disabled/></div>),
      //   width:82
      // },
      {  
        Header: () => (
    ...
    return (
      <>
        ...
        {/* Search */}
          <div className='container row mt-2'>
            <div className='col-10'>
              <input value={this.state.allCustomersSearchInput} onChange={this.allCustomersSearchInputChange} className='form-control w-100' type="search" placeholder='Search by email'/>
            </div>
            <div className='col-2'>
              <input className='form-control' type="submit" value='Search' onClick={this.allCustomersSearch}/>
            </div>
          </div>      
          <div className='container row mt-2'>
              {this.state.allCustomersSearchFieldEmptyError&&<h5 className="col-12" style={{color:"red"}}>Field is empty</h5>}
              {this.state.allCustomersSearchNotFoundError&&<h5 className="col-12" style={{color:"red"}}>Email not found</h5>}
          </div>
        {/* Search */}
        ...
        <EditPopup2 sureButtonClick={this.sureButton} cancelButtonClick={this.cancelButton} sureButtonRef1={this.sureButtonRef1}/>
        <ReactTable 
